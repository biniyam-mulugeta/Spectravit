...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # inputs: logits [B, C], targets: keys [B]
...
class SpectraLoss(nn.Module):
    """
    Unified Loss: Focal + SupCon + AuxFocal (Spectral) + L1/TV Reg
    """
    def __init__(self, 
                 lambda_focal=0.5, 
                 lambda_supcon=0.5, 
    ...
    def forward(self, logits_main, embeddings, spectral_feat, logits_aux, labels, current_aux_weight=0.0):
        ...
        # 3. Aux Spectral Focal
        if current_aux_weight > 0:
            loss_aux = self.focal(logits_aux, labels)
        else:
            loss_aux = torch.tensor(0.0, device=logits_main.device)

        # 4. Reg (L1 + TV)
        loss_reg = torch.tensor(0.0, device=logits_main.device)
        if self.lambda_l1 > 0:
            loss_reg = loss_reg + torch.mean(torch.abs(spectral_feat)) * self.lambda_l1
        if self.lambda_tv > 0:
            loss_reg = loss_reg + self._total_variation(spectral_feat) * self.lambda_tv

        # Weighted Sum
        total_loss = (self.lambda_main * loss_focal) + \
                     (self.lambda_supcon * loss_supcon) + \
                     (current_aux_weight * loss_aux) + \
                     loss_reg
