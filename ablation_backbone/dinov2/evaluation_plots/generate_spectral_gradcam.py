...
class AttentionRollout:
    def __init__(self, model):
        ...
        self.hook = model.backbone.blocks[-1].attn.attn_drop.register_forward_hook(self.save_attention)
...
def process_image(model, grad_cam, attn_extractor, image_path, output_dir, device):
    ...
    # Run a forward pass to specifically capture the attention map
    with torch.no_grad():
        model(input_tensor)

    # Grad-CAM (requires backward)
    cam_map, pred_idx = grad_cam(input_tensor) # [1, 1, H, W] (16x16)
    
    # Attention (captured during the explicit forward pass above)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_images", type=int, default=20, help="Maximum number of images to generate")
